...
class Consumer:
    ...
    def check_queue_exists(self):
        ...
        try:
            ...
            logging.exception(f"Queue '{self.config_dict_rmq['queue_name']}' does not exist.")
            raise
        except Exception as e:
            logging.exception(f"An error occurred while checking the queue: {e}")
            raise e
    ...
    def start_consumer(self, callback):
        while True:
            ...
            try:
                ...
                logging.exception(f"An error occurred while consuming messages: {e}")
                logging.exception("Reconnecting to RabbitMQ...")
                while i < self.retry_attempt:
                    try:
                        ...
                        logging.exception(f"Reconnection attempt {i + 1} failed: {e}")
                        time.sleep(5)
