...
class TestROIHandler(unittest.TestCase):
    ...
    def test_combine_rois_multiple(self):
        combined_mask = roi_handler.combine_rois(
            self.rtstruct, [ROI_PTV, ROI_CTV, ROI_VESSELS], [OPERATION_SUBTRACTION, OPERATION_ADDITION]
        )
    ...
    def test_combine_rois_invalid(self):
        with self.assertRaises(AssertionError):
            roi_handler.combine_rois(
                self.rtstruct, [ROI_KIDNEY_LEFT, ROI_KIDNEY_RIGHT], [OPERATION_ADDITION, OPERATION_SUBTRACTION]
            )
